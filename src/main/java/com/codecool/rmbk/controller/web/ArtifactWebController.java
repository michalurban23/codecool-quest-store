...
public class ArtifactWebController extends CommonHandler {
    ...
    private void handleWebArtifact(String accessLevel, String name, Map<String, String> sideMenu) throws IOException {
        ...
        if (accessLevel.equals("student")) {
            response = webDisplay.getSiteContent(name, sideMenu, prepareStudentOptions("artifact"));
            send200(response);

        } else if (accessLevel.equals("mentor")) {
            response = webDisplay.getSiteContent(name, sideMenu, prepareMentorOptions("artifact"));
            send200(response);
